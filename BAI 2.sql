...
CALL AddInventory(10 , -1);

DROP PROCEDURE AddInventory;

DELIMITER //
-- cho nên ta sẽ cho thêm if vào để báo 
CREATE PROCEDURE AddInventory(IN p_item_id INT , IN p_quantity INT)
BEGIN 
...
CALL AddInventory(10 , -1);
-- khi đã có if thì kể cả khi ta cho số -1 , hàm này sẽ vẫn chạy nhưng không gây ảnh hưởng gì đến 
-- giữ liệu cả cho đến khi nhập số dương
